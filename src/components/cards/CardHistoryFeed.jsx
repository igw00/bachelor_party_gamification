...
export default function CardHistoryFeed() {
  ...
  // Only show activated, non-secret cards — sorted newest first
  const activated = [...cards.filter((c) => c.activatedAt && !c.isSecret)]
    .sort((a, b) => (b.activatedAt?.seconds ?? 0) - (a.activatedAt?.seconds ?? 0))
    ...
    return (
      <div className="flex flex-col items-center justify-center py-16 text-center px-6">
        ...
        <p className="text-sm text-on-surface-variant mt-1">
          Activated cards will appear here — visible to everyone.
        </p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6 pt-2 pb-4">
      {activated.map((card, i) => {
        const team = teams.find((t) => t.id === card.drawnByTeamId)
        const player = players.find((p) => p.id === card.activatedByPlayerId)
        const teamInitials = team?.name?.split(' ').map((w) => w[0]).slice(0, 2).join('').toUpperCase() ?? '?'

        return (
          <div
            key={card.id}
            className="bg-surface-container-lowest rounded-2xl overflow-hidden shadow-[0_4px_16px_rgba(0,0,0,0.04)]"
          >
            {/* Post header */}
            <div className="flex items-center gap-3 px-4 py-3 border-b border-surface-container">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-on-surface-variant">
                  {player ? `${player.name} activated a card` : 'activated a card'}
                  {' · '}
                  {timeAgo(card.activatedAt?.seconds)}
                </p>
              </div>
              <span
                className={`text-[10px] font-black uppercase tracking-wider px-2.5 py-1 rounded-full flex-shrink-0
                  ${TYPE_BADGE[card.type] ?? 'bg-surface-container text-on-surface-variant'}`}
              >
                {card.type}
              </span>
            </div>
